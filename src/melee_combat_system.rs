...
impl<'a> System<'a> for MeleeCombatSystem {
  ...
  fn run(&mut self, data: Self::SystemData) {
    ...
    for e(_entity, wants_melee, name, stats) in (&entities, &wants_melee, &names, &combat_stats).join() {
      if stats.hp > 0 {
        ...
        if target_stats.hp > 0 {
          ...
          if damage == 0 {
            ...
            inflict_damage.insert(wants_melee.target, SufferDamage{ amount: damage }).expect("@ERROR: Unable to do damage");
          }
        }
      }
    }

    // Remove the wants_melee `message` component
    wants_melee.clear();
  }
}